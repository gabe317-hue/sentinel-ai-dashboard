 ...
 *   0. SOBRECOSTO — Cost overrun detected using SEFIN budget execution data
...
const { SEFINDataIntegration, detectSOBRECOSTO: detectSOBRECOSTOWithSEFIN } = require('./sefin-integration');
...
const PAGES_TO_FETCH            = 5;        // pages to fetch per run (10 contracts/page)
const SOBRECOSTO_THRESHOLD      = 0.15;     // 15% over budget triggers SOBRECOSTO flag
...
// silently ignored.
//
// Strategy: Persist a page counter in `pipeline_settings` (Supabase).
// Each nightly run fetches the next PAGES_TO_FETCH pages, advancing the
// counter.  After ~62,000 runs the full catalogue will have been processed.
// Each run reliably returns ~50 real contracts for fraud analysis.
//
...
async function fetchAllContracts(supabase) {
  // Read persisted page counter so each nightly run advances through the full
  // catalogue instead of re-fetching page 1 every time.
  let startPage = 1;
  if (supabase) {
    const stored = await getPipelineSetting(supabase, 'last_fetched_page');
    if (stored !== null) {
      startPage = Math.max(1, parseInt(stored, 10) + 1);
    }
  }

  const endPage = startPage + PAGES_TO_FETCH - 1;
  console.log(`[Pipeline] Fetching pages ${startPage}–${endPage} sequentially...`);
  ...
  for (let pagina = startPage; pagina <= endPage; pagina++) {
    ...
    if (!data) {
      console.warn(`[Pipeline] Page ${pagina} returned no data — stopping early`);
      break; // don't advance counter past a failed page
    }
    ...
    lastSuccessPage = pagina;
  }

  // Persist the last page we successfully fetched
  if (supabase && lastSuccessPage >= startPage) {
    await updatePipelineSetting(supabase, 'last_fetched_page', lastSuccessPage);
    console.log(`[Pipeline] Page counter saved: ${lastSuccessPage}`);
  }
...
// ── Pipeline settings (pagination state) ─────────────────────
// Stores key/value pairs in a `pipeline_settings` Supabase table so state
// persists across nightly runs.  Table DDL:
//   CREATE TABLE IF NOT EXISTS pipeline_settings (
//     key TEXT PRIMARY KEY,
//     value TEXT,
//     updated_at TIMESTAMPTZ DEFAULT NOW()
//   );
//   INSERT INTO pipeline_settings (key, value) VALUES ('last_fetched_page', '0')
//     ON CONFLICT (key) DO NOTHING;

async function getPipelineSetting(supabase, key) {
  ...
  if (error) {
    console.warn(`[Pipeline] Could not read setting "${key}":`, error.message);
    return null;
  }
  return data?.value ?? null;
}

async function updatePipelineSetting(supabase, key, value) {
  const { error } = await supabase
    .from('pipeline_settings')
    .upsert({ key, value: String(value), updated_at: new Date().toISOString() }, { onConflict: 'key' });

  if (error) {
    console.warn(`[Pipeline] Could not update setting "${key}":`, error.message);
  }
}
...
function normalizeContract(raw) {
  ...
  // Extract original budget (presupuesto) — OCDS planning.budget.amount
  const presupuesto = parseFloat(
    raw.planning?.budget?.amount?.amount ||
    raw.planning?.budget?.amount         ||
    0
  );

  // Extract actual paid amount (monto_pagado) from implementation.transactions[].value.amount
  // This represents the actual payment made, which may differ from the contracted amount
  const monto_pagado = parseFloat(
    (raw.implementation?.transactions && raw.implementation.transactions.length > 0)
      ? raw.implementation.transactions[0].value?.amount
      : 0
  );
  ...
  return {
    ...
    presupuesto:        Math.min(Number(presupuesto) || 0, 99_999_999_999),
  };
}

function detectPatterns(contract, vendorCounts = {}, sanctionsList = [], sefinIntegration = null) {
  const patterns = [];
  const { monto, monto_pagado, tipo_contratacion, fecha_publicacion, fecha_adjudicacion, proveedor_id } = contract;
  ...
  // Pattern 0b: SOBRECOSTO — cost overrun detection (with SEFIN budget data)
  // Primary: SEFIN budget execution data (actual payments)
  // Fallback 1: EDCA payment data (monto_pagado)
  // Fallback 2: Budget comparison (presupuesto)
  // All require contract amount >= L.100K
  const sobrecostoPattern = detectSOBRECOSTOWithSEFIN(contract, sefinIntegration, SOBRECOSTO_THRESHOLD);
  if (sobrecostoPattern) {
    patterns.push(sobrecostoPattern);
  }
...
function buildAlertRecord(contract, patterns, severity) {
  ...
  return {
    ...
    monto_pagado: Math.min(Number(contract.monto_pagado) || 0, 99_999_999_999), // actual paid amount from implementation.transactions
    budget_amount: contract.presupuesto > 0 ? contract.presupuesto : null, // original budget from planning.budget
...
async function insertAlerts(supabase, alerts) {
  ...
  for (let i = 0; i < alerts.length; i += BATCH_SIZE) {
    ...
    const { data: upsertData, error } = await supabase
      .from('alerts')
      .upsert(batch, { onConflict: 'contract_id', ignoreDuplicates: true })
      .select('id');

    if (error) {
      console.error(`[Pipeline] Upsert error (batch ${Math.floor(i/BATCH_SIZE) + 1}):`, error.message);
      errors += batch.length;
    } else {
      const batchInserted = (upsertData || []).length;
      inserted += batchInserted;
      console.log(`[Pipeline] Upserted batch ${Math.floor(i/BATCH_SIZE) + 1}: ${batchInserted} new alerts (${batch.length - batchInserted} skipped as duplicates)`);
    }
  }
...
async function runPipeline() {
  ...
  // Step 0b: Initialize SEFIN budget data integration for SOBRECOSTO detection
  console.log('[SEFIN] Initializing budget data integration...');
  const sefinIntegration = new SEFINDataIntegration('/tmp/sefin-cache');
  await sefinIntegration.fetchSEFINData();
  console.log(`[SEFIN] Ready with ${sefinIntegration.budgetData.length} budget records`);

  // Step 1: Fetch contracts from HonduCompras — advances page counter each run
  const rawContracts = await fetchAllContracts(supabase);
  ...
  for (const contract of newContracts) {
    const patterns = detectPatterns(contract, vendorCounts, sanctionsList, sefinIntegration);
