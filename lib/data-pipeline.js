...
//
// The HonduCompras releases endpoint paginates 10 records per page with
// 60,000+ total pages. No date filter is supported. We fetch pages 1-5
// in parallel through the Cloudflare Worker proxy — fast, reliable, and
// always returns real contracts regardless of the archive's date coverage.
//
...
async function fetchAllContracts() {
  console.log(`[Pipeline] Fetching ${PAGES_TO_FETCH} pages in parallel from SEFIN...`);

  // Fetch pages 1–PAGES_TO_FETCH simultaneously
  const pageResults = await Promise.all(
    Array.from({ length: PAGES_TO_FETCH }, (_, i) => fetchReleasesPage(i + 1))
  );
  ...
  for (let i = 0; i < pageResults.length; i++) {
    ...
    if (!data) {
      console.warn(`[Pipeline] Page ${i + 1} returned no data`);
      continue;
    }
    const releases = data.releasePackage?.releases || data.releases || [];
    console.log(`[Pipeline] Page ${i + 1}: ${releases.length} contracts`);
