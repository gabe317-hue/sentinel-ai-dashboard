...
//
// The HonduCompras releases endpoint paginates 10 records per page,
// oldest-first. Page 1 = contracts from ~2013. The API response includes
// a `pages` field with the total page count (150,000+). We fetch the
// LAST N pages to get the most recent contracts.
//
// Strategy:
//   1. Fetch page 1 → read `data.pages` (total page count)
//   2. Fetch pages (total-N+1) through total → newest contracts
//
...
async function fetchAllContracts() {
  // Step 1: Get total page count from page 1
  console.log(`[Pipeline] Fetching page 1 to discover total page count...`);
  const firstPageData = await fetchReleasesPage(1);
  if (!firstPageData) {
    console.warn('[Pipeline] Could not reach SEFIN API. Check Worker proxy.');
    return [];
  }
  const totalPages = firstPageData.pages || 1;
  console.log(`[Pipeline] Total pages in SEFIN: ${totalPages.toLocaleString()}`);

  // Step 2: Fetch the LAST N pages — these contain the most recent contracts
  // (API is sorted oldest-first; page 1 = ~2013, last page = today)
  const startPage = Math.max(1, totalPages - PAGES_TO_FETCH + 1);
  console.log(`[Pipeline] Fetching pages ${startPage}–${totalPages} (newest contracts)...`);
  ...
  const allContracts = [];
  for (let pagina = startPage; pagina <= totalPages; pagina++) {
...
function buildAlertRecord(contract, patterns, severity) {
  ...
  return {
    ...
    monto_contrato: contract.monto || 0,     // dashboard reads a.monto_contrato
